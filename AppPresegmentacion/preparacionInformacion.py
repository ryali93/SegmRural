...
# def leerExcel(tipo, excel=r'D:\SegmentacionRuralV2\Procesamiento\Ubigeos_Procesar\UBIGEOS_14-06-17.xlsx', nombreHoja = "UBIGEOS"):
def leerExcel(tipo, excel=r'D:\SegmentacionRuralV2\Procesamiento\Ubigeos_Carga\UBIGEOS_APTOS_29-06-17.xlsx', nombreHoja = "UBIGEOS"):
...
def ubigeosCcpp(conn=conn):
    ...
    cursor.execute("SELECT DISTINCT UBIGEO FROM CPV_SEGMENTACION_GDB.sde.TB_CCPP WHERE ISNULL (VIV_CCPP,0) = 0 AND AREA = 2")
    ubigeos = [x[0] for x in cursor]
    cursor.close()
    return ubigeos
...
def ubigeosCcppAer(conn=conn):
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT UBIGEO FROM CPV_SEGMENTACION_GDB.sde.TB_CCPP WHERE (AER_INI IS NULL) AND (AREA = 2)")
    ubigeos = [x[0] for x in cursor]
    cursor.close()
    return ubigeos
...
def actualizaCpppVacios(ubigeos, conn=conn):
    for ubigeo in ubigeos:
        cursor = conn.cursor()
        cursor.execute("UPDATE CPV_SEGMENTACION_GDB.sde.TB_CCPP SET VIV_CCPP = 0 WHERE (VIV_CCPP IS NULL) AND AREA = 2".format(ubigeo))
        conn.commit()
        cursor.close()
...
def asignarAer(ubigeos, TB_AER, conexion=conexionDB, conn=conn):

    asignarIDAER()
    # aers = r'{}\CPV_SEGMENTACION_GDB.sde.TB_AER'.format(conexion)
    aers = TB_AER


    for ubigeo in ubigeos:
        print ubigeo
        ccppsmfl = arcpy.MakeQueryLayer_management(conexion, 'CCPP', "SELECT*FROM CPV_SEGMENTACION_GDB.sde.TB_CCPP WHERE UBIGEO = '{}' AND AREA = 2".format(ubigeo), 'LLAVE_CCPP', 'POINT', '4326', arcpy.SpatialReference(4326))
        ...
        for aer in arcpy.da.SearchCursor(aers, ["SHAPE@", "AER_INI", "AER_FIN", "IDAER"], "UBIGEO = '{}'".format(ubigeo)):
            ...
            if len(listaCcpps) > 0:
                ...
                cursor = conn.cursor()
                cursor.execute("UPDATE CPV_SEGMENTACION_GDB.sde.TB_CCPP SET AER_INI = '{}', AER_FIN = '{}', IDAER = '{}' WHERE {}".format(aer[1], aer[2], aer[3], expresion))
                conn.commit()
                cursor.close()
            else:
                pass
...
def asignarCCPPAER(tipo, ubigeos_input,TB_AER):
    if tipo == 1 and len(ubigeos_input) > 0:
        ubigeosnacional = ubigeos_input
    ...
    print ubigeos
    asignarAer(ubigeos,TB_AER)


def procesarInformacionPreSegmentacion(tipo, ubigeos_input,TB_AER):
    asignarCantidadViviendas(tipo, ubigeos_input)
    asignarCcppVacios(tipo, ubigeos_input)
    asignarCCPPAER(tipo, ubigeos_input,TB_AER)
