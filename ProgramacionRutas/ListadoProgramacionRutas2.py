...
def infoEmpadronador(ubigeo, conn=conn):
    cursor = conn.cursor()
    cursor.execute("SELECT EMP, IDEMP, SCR, IDRUTA, IDSCR FROM CPV_SEGMENTACION.dbo.SEGM_R_EMP WHERE UBIGEO = '{}' AND FASE  = 'CPV2017' ORDER BY EMP ".format(ubigeo))
...
def infoEmpadronadorSCR(ubigeo, conn=conn):
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT IDSCR, EMP FROM CPV_SEGMENTACION.dbo.SEGM_R_EMP WHERE UBIGEO = '{}' AND FASE  = 'CPV2017'".format(ubigeo))
...
def infoSCR(ubigeo, conn=conn):
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT IDSCR FROM CPV_SEGMENTACION.dbo.SEGM_R_EMP WHERE UBIGEO = '{}' AND FASE  = 'CPV2017'".format(ubigeo))
